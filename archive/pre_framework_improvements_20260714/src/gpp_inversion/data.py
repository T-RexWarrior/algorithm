...
class MultiStationWindowDataset(Dataset):
    ...
    def _load_station(self, path: Path) -> None:
        ...
        if self.features.time not in frame.columns:
            return
        missing = [column for column in self.features.required if column not in frame.columns]
        if missing:
            return
    ...
    def _build_windows(self) -> None:
        ...
        for station_index, dates in enumerate(self.station_dates):
            timestamps = pd.to_datetime(dates)
            differences = np.diff(timestamps.values).astype("timedelta64[s]").astype(float) / 3600.0
            positive = differences[differences > 0]
            expected_step = float(pd.Series(positive).mode().iloc[0]) if positive.size else 0.0
            for start in range(len(dates) - length + 1):
                window_differences = differences[start : start + length - 1]
                if window_differences.size != length - 1:
                    continue
                if self.window.require_regular and not np.allclose(
                    window_differences, expected_step
                ):
                    continue
                if self.window.max_gap_hours is not None and np.any(
                    window_differences > self.window.max_gap_hours
                ):
                    continue
                span = (
                    timestamps[start + length - 1] - timestamps[start]
                ).total_seconds() / 3600.0
                if self.window.max_span_hours is not None and span > self.window.max_span_hours:
                    continue
                self.samples.append((station_index, start))

    def __len__(self) -> int:
        return len(self.samples)

    def __getitem__(self, index: int):
        station_index, start = self.samples[index]
        ...
        dates = pd.to_datetime(self.station_dates[station_index][start:end])
        ...
        if self.window.time_features is not TimeFeatureMode.CYCLIC:
            ...
            target_time = dates[-1]
            age = np.asarray(
                [(target_time - value).total_seconds() / 3600.0 for value in dates],
                dtype=np.float32,
            )
            age = np.clip(
            ...
            if self.window.time_features is TimeFeatureMode.CDE:
                relative = np.asarray(
                    [(value - dates[0]).total_seconds() / 3600.0 for value in dates],
                    dtype=np.float32,
                )
