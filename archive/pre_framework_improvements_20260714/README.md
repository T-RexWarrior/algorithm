...
- `src/gpp_inversion/` 包含统一数据集、站点划分、缩放、损失函数、TCN/Mamba/Neural CDE 模型、训练评估引擎和完整实验管线。
...
旧 `main.py` 把同一批 CSV 同时作为训练、验证和测试数据，而且数据集构造函数曾删除 `split_type` 参数后仍被旧入口传入。为避免误用，这个入口只保留在 `archive/`。新的统一管线会先检查站点互斥性，只用训练集拟合缩放器，再将同一缩放器传给验证和测试。

## 统一运行方式

复制并修改 `configs/experiment.example.json` 后运行：

```powershell
python scripts/run_experiment.py configs/your_experiment.json
```

也可以打开 `notebooks/00_integrated/统一实验入口.ipynb`。模型、时间特征、缩放和损失函数均通过配置选择。
